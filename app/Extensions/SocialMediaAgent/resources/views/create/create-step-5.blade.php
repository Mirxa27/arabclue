...
>
    <h2 class="mb-9 text-center text-[24px] font-medium leading-[1.2em]">
        <span class="block text-[0.875em] opacity-50">
            @lang('Let’s fine tune your strategy.')
        </span>
    ...
    <div class="flex flex-col gap-5 rounded-[20px] border px-5 py-7">
        ...
        <div class="space-y-3">
            <x-forms.input
                ...
                switcher
                switcherFill
                ::checked="formData.include_hashtags || formData.include_emoji"
                @change="formData.include_hashtags = formData.include_emoji = $el.checked"
            />

            <div>
                <label
                    class="flex items-center justify-between gap-1 text-xs font-medium text-heading-foreground"
                    x-show="formData.include_hashtags"
                    x-collapse
                >
                    @lang('Number of Hashtags')

                    <input
                        class="lqd-input-stepper h-8 w-14 shrink-0 rounded-md border text-center"
                        type="number"
                        min="0"
                        max="30"
                        x-model.number="formData.hashtag_count"
                    />
                </label>
            </div>
        ...
        >
            ...
            >
                ...
                <p
                    class="mb-0 text-xs font-medium text-heading-foreground"
                    x-text="plans[formData.plan_type]"
                ></p>
                <x-tabler-chevron-down class="absolute end-4 top-1/2 size-4 -translate-y-1/2" />
            </div>
            ...
            >
                <x-forms.input
                    ...
                    type="search"
                    x-model="searchStr"
                    placeholder="{{ __('Search for plans') }}"
                />
                @foreach ($plans as $key => $label)
                    <button
                        ...
                        type="button"
                        x-show="searchStr === '' || $el.dataset.key.toLowerCase().includes(searchStr.toLowerCase()) || $el.dataset.label.toLowerCase().includes(searchStr.toLowerCase())"
                        @click.prevent="formData.plan_type = '{{ $key }}'; dropdownOpen = false;"
                    >
                        {{ $label }}
                    </button>
                @endforeach
    ...
    <div class="mt-2">
        @include('social-media-agent::create.step-error', ['step' => 5])
    </div>
