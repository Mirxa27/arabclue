...
@forelse($posts as $post)
    ...
    <div
        ...
        x-data='socialMediaAgentPostItem({ postId: {{ $post->id }}, status: "{{ $post->status }}", media_urls: @json($post->media_urls), platform_id: {{ $post->platform_id }}, platform: @json($post->platform), post_type: "{{ $post->post_type }}", scheduled_at: "{{ $post->scheduled_at }}", content: @json($post->content), editSidedrawerId: "#social-media-agent-sidedrawer" })'
    >
        <div class="flex grow flex-col rounded-[10px] border p-3.5 transition">
            <div class="mb-2.5 flex items-center justify-between gap-1">
                ...
                <span class="whitespace-nowrap text-3xs opacity-55">
                    <span
                        ...
                        x-text="post_type"
                    >
                        {{ $post->post_type ? Str::headline($post->post_type) : __('Post') }}
                    </span>
            ...
            <template x-if="media_urls.length">
                <figure
                    ...
                    x-data="{
                        totalSlides: media_urls?.length ?? 0,
                        currentIndex: 0,
                        prev() {
